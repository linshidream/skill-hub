...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `dev-lifecycle` | `0.1.5` | `dev` | 可用 | Claude Code / Codex / Generic | 2026-06-12 21:30 +0800 | 2026-07-09 14:30 +0800 | 可中断恢复地编排业务闭环级步骤开发、实时 step 状态、构建失败回修、分支生命周期和 Jenkins 测试部署流程；状态按功能隔离支持多功能并行，活动指针以当前分支为准同步；GUI merge 作为默认关闭的可选辅助能力，失败时降级到文本冲突流程。 | `skills/dev/dev-lifecycle` |
