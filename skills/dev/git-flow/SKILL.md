...
GUI merge 是非核心辅助能力，不能成为 lifecycle 主流程的前置条件。默认关闭；关闭或不可用时，必须自动降级到文本冲突流程。
...
1. 只有 `integration.conflict.gui-merge.enabled=true` 时才考虑 GUI merge。
2. `tool` 默认 `intellij`，`command` 默认 `idea`。
3. 冲突后先检测：
   - `command -v <command>` 是否存在，`command` 可以是 `idea` 或 IDEA 可执行文件完整路径。
   - `git config --get mergetool.intellij.cmd` 是否已配置。
4. 检测通过时，冲突报告给出：
...
5. 任一检测失败时，冲突报告必须写明原因，并自动 fallback 到文本冲突流程，不中断主流程。

IntelliJ IDEA mergetool 配置：
...
如果 `idea` 命令不可用，先安装 IDEA command-line launcher，或把 IDEA 可执行目录加入 `$PATH`。

如果当前 Agent 进程尚未继承 `$PATH`，也允许显式配置完整路径：
...
1. Agent 展示冲突文件、冲突块和双方意图，等待人工决策。
2. 对出现 `<<<<<<<` / `=======` / `>>>>>>>` 的文件，必须按三方语义处理：
...
4. 只有确认无冲突标记、无语法/格式问题后，才允许 `git add` 和继续 merge commit。
