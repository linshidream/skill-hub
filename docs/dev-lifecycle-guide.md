...
| `integration.conflict.gui-merge.enabled` | 是否启用可视化 merge，默认 false；这是辅助能力，不影响主流程 |
...
### 3.5 可选：配置 IDEA 可视化 merge

GUI merge 是辅助能力，默认关闭。未配置或检测失败时，流程自动降级到文本冲突处理，不影响 dev-lifecycle 主流程。

如果要启用 IDEA merge，先确认 `idea` 命令可用：
...
如果没有 `idea` 命令，在 IntelliJ IDEA 中创建 command-line launcher，或把 IDEA 可执行目录加入 `$PATH`。macOS 示例：
...
需要长期生效时，可写入 shell 启动文件，然后重启 Terminal 或当前 Agent/Codex 进程：

```bash
echo 'export PATH="/Applications/IntelliJ IDEA 2.app/Contents/MacOS:$PATH"' >> ~/.zshrc
idea --version
```

然后配置 Git mergetool。推荐使用 `idea` 命令：
...
git config --global --get mergetool.intellij.cmd
```
...
如果 `idea` 命令在当前 Agent 进程中仍不可见，但 IDEA 可执行文件存在，也可以使用完整路径兜底：
...
如果使用完整路径兜底，`.dev-flow.yml` 也要同步改成同一条可执行路径：
...
integration:
  conflict:
    gui-merge:
      enabled: true
      tool: intellij
      command: "/Applications/IntelliJ IDEA 2.app/Contents/MacOS/idea"
      fallback: text
```

冲突时，`git-flow` 会检测 `command` 指向的本地命令和 `mergetool.intellij.cmd`。检测通过才提示：

```bash
git mergetool --tool intellij
```

检测失败时会在冲突报告中说明原因，并自动降级到文本冲突流程。
...
如果已开启 `integration.conflict.gui-merge.enabled=true`，且冲突报告中 `gui_merge.available=true`，Agent 可以提示：

```bash
git mergetool --tool intellij
```

如果 `gui_merge.available=false`，不要卡住流程，直接使用文本冲突处理。
